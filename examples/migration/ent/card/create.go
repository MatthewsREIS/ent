...
func (_c *CardCreate) AddPaymentIDs(ids ...int) *CardCreate {
	_ = _c.mutation.AddEdgeIDs("payments", entbuilder.ToAny(ids)...)
	return _c
}
...
func (_c *CardCreate) defaults() {
	if _, ok := entbuilder.GetField[string](_c.mutation, "type"); !ok {
		v := DefaultType
		_ = _c.mutation.SetField("type", v)
	}
...
func (_c *CardCreate) check() error {
	...
	if _, ok := entbuilder.GetField[string](_c.mutation, "number_hash"); !ok {
		return &ValidationError{Name: "number_hash", Err: errors.New(`ent: missing required field "Card.number_hash"`)}
	}
	if _, ok := entbuilder.GetField[string](_c.mutation, "cvv_hash"); !ok {
...
func (_c *CardCreate) createSpec() (*Card, *sqlgraph.CreateSpec) {
	...
	if value, ok := entbuilder.GetField[string](_c.mutation, "number_hash"); ok {
		_spec.SetField(FieldNumberHash, field.TypeString, value)
		_node.NumberHash = value
	}
	if value, ok := entbuilder.GetField[string](_c.mutation, "cvv_hash"); ok {
	...
	if nodes := entbuilder.EdgeIDsAs[int](_c.mutation, "payments"); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   PaymentsTable,
