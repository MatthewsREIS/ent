...
type PaymentQuery struct {
	Config
	entbuilder.QueryState[predicate.Payment]
	order        []OrderOption
	eagerLoaders map[string]func(context.Context, []*Payment) error
}
...
func (_q *PaymentQuery) StoreEager(name string, loader func(context.Context, []*Payment) error) *PaymentQuery {
	if _q.eagerLoaders == nil {
		_q.eagerLoaders = make(map[string]func(context.Context, []*Payment) error)
	}
...
func (_q *PaymentQuery) EagerLoaders() map[string]func(context.Context, []*Payment) error {
...
func (_q *PaymentQuery) Order(o ...OrderOption) *PaymentQuery {
...
func (_q *PaymentQuery) First(ctx context.Context) (*Payment, error) {
	_q.Limit(1)
	return entbuilder.RunFirst[*Payment, []*Payment](ctx, _q, _q.Ctx, ent.OpQueryFirst, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Payment, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} })
}
...
func (_q *PaymentQuery) Only(ctx context.Context) (*Payment, error) {
	_q.Limit(2)
	return entbuilder.RunOnly[*Payment, []*Payment](ctx, _q, _q.Ctx, ent.OpQueryOnly, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Payment, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} }, func(label string) error { return &NotSingularError{Label: label} })
}
...
func (_q *PaymentQuery) All(ctx context.Context) ([]*Payment, error) {
	return entbuilder.RunAll[[]*Payment](ctx, _q, _q.Ctx, ent.OpQueryAll, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Payment, error) { return _q.sqlAll(ctx) })
}
...
func (_q *PaymentQuery) Clone() *PaymentQuery {
	...
	return &PaymentQuery{
		...
		eagerLoaders: cloneEagerLoadersPayment(_q.eagerLoaders),
	}
}
...
// dispatch loop.
func (_q *PaymentQuery) Fetch(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	var (
		nodes       = []*Payment{}
		_spec       = _q.querySpec()
		loadedTypes = [1]bool{
...
func (_q *PaymentQuery) IncludeForeignKeys(bool) *PaymentQuery {
	return _q
}
...
func (_q *PaymentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.Ctx.Fields; len(fields) > 0 {
		...
		if _, ok := _q.eagerLoaders["card"]; ok {
			_spec.Node.AddColumnOnce(FieldCardID)
		}
	}
...
func (_g *PaymentGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*PaymentQuery, *PaymentGroupBy](ctx, _g.build, _g, _g.build.QueryState.Inters, v)
}
...
func (_s *PaymentSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*PaymentQuery, *PaymentSelect](ctx, _s.PaymentQuery, _s, _s.QueryState.Inters, v)
}
...
// paymentCreateDescriptor holds the metadata and callbacks for constructing a Payment entity.
var paymentCreateDescriptor = &entbuilder.CreateDescriptor[Config, Payment, *PaymentMutation]{
	Table:   Table,
	NewNode: func(c Config) *Payment { return &Payment{Config: c} },
	ID: &entbuilder.IDDescriptor[Config, Payment, *PaymentMutation]{
		Column:      FieldID,
		Type:        field.TypeInt,
		UserDefined: false,
		AssignGenerated: func(n *Payment, v driver.Value) error {
			id, ok := v.(int64)
			if !ok {
				return fmt.Errorf("unexpected Payment.ID type: %T", v)
			}
			n.ID = int(id)
			return nil
		},
...
// cloneEagerLoadersPayment returns a shallow copy of the eager-loaders map.
// The closures themselves are shared between original and clone.
func cloneEagerLoadersPayment(in map[string]func(context.Context, []*Payment) error) map[string]func(context.Context, []*Payment) error {
	if in == nil {
		return nil
	}
	out := make(map[string]func(context.Context, []*Payment) error, len(in))
	for k, v := range in {
		out[k] = v
	}
	return out
