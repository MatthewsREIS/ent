...
// LoadGroupTenant performs the eager-load for the "tenant" edge. Body mirrors
// the pre-PR6 *GroupQuery.loadTenant method, hoisted to root
// so it can reference cross-package types directly.
func LoadGroupTenant(ctx context.Context, query *tenant.TenantQuery, nodes []*group.Group) error {
...
// WithGroupTenant eager-loads the "tenant" edge on a group.GroupQuery. The
// optional arguments configure the sibling sub-query before storage.
func WithGroupTenant(q *group.GroupQuery, opts ...func(*tenant.TenantQuery)) *group.GroupQuery {
	sub := tenant.NewTenantClient(q.Config).Query()
	for _, opt := range opts {
		opt(sub)
	}
	return q.StoreEager("tenant", func(ctx context.Context, parents []*group.Group) error {
		return LoadGroupTenant(ctx, sub, parents)
	})
}

// QueryGroupTenant returns a tenant.TenantQuery for the "tenant" edge of a given group.Group.
func QueryGroupTenant(c *group.GroupClient, _m *group.Group) *tenant.TenantQuery {
...
// QueryGroupTenantFromQuery returns a tenant.TenantQuery that traverses the "tenant" edge
// of every group.Group matched by q (chained-query form). Mirrors the pre-PR6
// (*group.GroupQuery).QueryTenant method, hoisted to root so it
// can reference the cross-package tenant.TenantQuery type.
func QueryGroupTenantFromQuery(q *group.GroupQuery) *tenant.TenantQuery {
