...
func (_u *UserUpdate) AddManageIDs(ids ...int) *UserUpdate {
	_ = _u.mutation.AddEdgeIDs("manage", entbuilder.ToAny(ids)...)
	return _u
}
...
func (_u *UserUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.EdgeCleared("manage") {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
			Table:   ManageTable,
	...
	if nodes := _u.mutation.RemovedEdgeIDs("manage"); len(nodes) > 0 && !_u.mutation.EdgeCleared("manage") {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
			Table:   ManageTable,
	...
	if nodes := _u.mutation.EdgeIDs("manage"); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
			Table:   ManageTable,
