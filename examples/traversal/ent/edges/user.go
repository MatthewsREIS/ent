...
// LoadUserManage performs the eager-load for the "manage" edge. Body mirrors
// the pre-PR6 *UserQuery.loadManage method, hoisted to root
// so it can reference cross-package types directly.
func LoadUserManage(ctx context.Context, query *group.GroupQuery, nodes []*user.User) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*user.User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
		nodes[i].Edges.Manage = []*group.Group{}
	}
	query.IncludeForeignKeys(true)
...
// WithUserManage eager-loads the "manage" edge on a user.UserQuery. The
// optional arguments configure the sibling sub-query before storage.
func WithUserManage(q *user.UserQuery, opts ...func(*group.GroupQuery)) *user.UserQuery {
	sub := group.NewGroupClient(q.Config).Query()
	for _, opt := range opts {
		opt(sub)
	}
	return q.StoreEager("manage", func(ctx context.Context, parents []*user.User) error {
		return LoadUserManage(ctx, sub, parents)
	})
}

// QueryUserManage returns a group.GroupQuery for the "manage" edge of a given user.User.
func QueryUserManage(c *user.UserClient, _m *user.User) *group.GroupQuery {
	query := group.NewGroupClient(c.Config).Query()
	query.Path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryUserManageFromQuery returns a group.GroupQuery that traverses the "manage" edge
// of every user.User matched by q (chained-query form). Mirrors the pre-PR6
// (*user.UserQuery).QueryManage method, hoisted to root so it
// can reference the cross-package group.GroupQuery type.
func QueryUserManageFromQuery(q *user.UserQuery) *group.GroupQuery {
	query := group.NewGroupClient(q.Config).Query()
	query.Path = func(ctx context.Context) (fromV *sql.Selector, err error) {
		if err := q.PrepareQuery(ctx); err != nil {
			return nil, err
