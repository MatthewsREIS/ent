...
// LoadGroupAdmin performs the eager-load for the "admin" edge. Body mirrors
// the pre-PR6 *GroupQuery.loadAdmin method, hoisted to root
// so it can reference cross-package types directly.
func LoadGroupAdmin(ctx context.Context, query *user.UserQuery, nodes []*group.Group) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*group.Group)
	for i := range nodes {
		if nodes[i].GetGroupAdmin() == nil {
			continue
		}
		fk := *nodes[i].GetGroupAdmin()
	...
	for _, n := range neighbors {
		...
		for i := range parents {
			parents[i].Edges.Admin = n
		}
	}
	return nil
}

// WithGroupAdmin eager-loads the "admin" edge on a group.GroupQuery. The
// optional arguments configure the sibling sub-query before storage.
func WithGroupAdmin(q *group.GroupQuery, opts ...func(*user.UserQuery)) *group.GroupQuery {
	sub := user.NewUserClient(q.Config).Query()
	for _, opt := range opts {
		opt(sub)
	}
	return q.StoreEager("admin", func(ctx context.Context, parents []*group.Group) error {
		return LoadGroupAdmin(ctx, sub, parents)
	})
}

// QueryGroupAdmin returns a user.UserQuery for the "admin" edge of a given group.Group.
func QueryGroupAdmin(c *group.GroupClient, _m *group.Group) *user.UserQuery {
	query := user.NewUserClient(c.Config).Query()
	query.Path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
// QueryGroupAdminFromQuery returns a user.UserQuery that traverses the "admin" edge
// of every group.Group matched by q (chained-query form). Mirrors the pre-PR6
// (*group.GroupQuery).QueryAdmin method, hoisted to root so it
// can reference the cross-package user.UserQuery type.
func QueryGroupAdminFromQuery(q *group.GroupQuery) *user.UserQuery {
	query := user.NewUserClient(q.Config).Query()
	query.Path = func(ctx context.Context) (fromV *sql.Selector, err error) {
		if err := q.PrepareQuery(ctx); err != nil {
			return nil, err
