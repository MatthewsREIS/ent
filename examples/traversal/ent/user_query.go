...
var userManageEdgeLoadDescriptor = entbuilder.EdgeLoadDescriptor[User, Group, int, int]{
	EdgeSpec: func() *sqlgraph.EdgeSpec {
		return entbuilder.NewEdgeSpec(entbuilder.EdgeSpecParams{
			Rel:          sqlgraph.O2M,
			Inverse:      true,
			Table:        user.ManageTable,
			Columns:      user.ManageColumn,
			Bidi:         false,
			TargetColumn: group.FieldID,
			TargetType:   field.TypeInt,
		})
...
func (_q *UserQuery) loadManage(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	query.withFKs = true
	return entbuilder.LoadEdgeO2M(ctx, &userManageEdgeLoadDescriptor, nodes, init, assign,
		func(bool) {},
		func(fn func(*sql.Selector)) { query.Where(fn) },
		query.All)
	return nil
