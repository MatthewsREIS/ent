// Copyright 2019-present Facebook Inc. All rights reserved.
// This source code is licensed under the Apache 2.0 license found
// in the LICENSE file in the root directory of this source tree.

// Code generated by ent, DO NOT EDIT.

package street

import (
	"context"
	"errors"
	"fmt"

	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/runtime/entbuilder"
	"entgo.io/ent/schema/field"
)

// StreetCreate is the builder for creating a Street entity.
type StreetCreate struct {
	Config
	mutation *StreetMutation
	hooks    []Hook
}

// NewStreetCreate returns a new StreetCreate initialized with the given config, hooks, and mutation.
func NewStreetCreate(c Config, hooks []Hook, mutation *StreetMutation) *StreetCreate {
	return &StreetCreate{Config: c, hooks: hooks, mutation: mutation}
}

// SetName sets the "name" field.
func (_c *StreetCreate) SetName(v string) *StreetCreate {
	_ = _c.mutation.SetField("name", v)
	return _c
}

// SetCityID sets the "city" edge to the City entity by ID.
func (_c *StreetCreate) SetCityID(id int) *StreetCreate {
	_ = _c.mutation.SetEdgeID("city", id)
	return _c
}

// SetNillableCityID sets the "city" edge to the City entity by ID if the given value is not nil.
func (_c *StreetCreate) SetNillableCityID(id *int) *StreetCreate {
	if id != nil {
		_c = _c.SetCityID(*id)
	}
	return _c
}

// Mutation returns the StreetMutation object of the builder.
func (_c *StreetCreate) Mutation() *StreetMutation {
	return _c.mutation
}

// Save creates the Street in the database.
func (_c *StreetCreate) Save(ctx context.Context) (*Street, error) {
	return WithHooks(ctx, _c.sqlSave, _c.mutation, _c.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (_c *StreetCreate) SaveX(ctx context.Context) *Street {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *StreetCreate) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *StreetCreate) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}

// check runs all checks and user-defined validators on the builder.
func (_c *StreetCreate) check() error {
	if _, ok := entbuilder.GetField[string](_c.mutation, "name"); !ok {
		return &ValidationError{Name: "name", Err: errors.New(`ent: missing required field "Street.name"`)}
	}
	return nil
}

func (_c *StreetCreate) sqlSave(ctx context.Context) (*Street, error) {
	if err := _c.check(); err != nil {
		return nil, err
	}
	_node, _spec := _c.createSpec()
	if err := sqlgraph.CreateNode(ctx, _c.Drv, _spec); err != nil {
		if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{Msg: err.Error(), Wrap: err}
		}
		return nil, err
	}
	id := _spec.ID.Value.(int64)
	_node.ID = int(id)
	_c.mutation.SetID(_node.ID)
	_c.mutation.SetDone()
	return _node, nil
}

func (_c *StreetCreate) createSpec() (*Street, *sqlgraph.CreateSpec) {
	var (
		_node = &Street{Config: _c.Config}
		_spec = sqlgraph.NewCreateSpec(Table, sqlgraph.NewFieldSpec(FieldID, field.TypeInt))
	)
	if value, ok := entbuilder.GetField[string](_c.mutation, "name"); ok {
		_spec.SetField(FieldName, field.TypeString, value)
		_node.Name = value
	}
	if nodes := entbuilder.EdgeIDsAs[int](_c.mutation, "city"); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   CityTable,
			Columns: []string{CityColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec("id", field.TypeInt),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges = append(_spec.Edges, edge)
	}
	return _node, _spec
}

// StreetCreateBulk is the builder for creating many Street entities in bulk.
type StreetCreateBulk struct {
	Config
	err      error
	builders []*StreetCreate
}

// NewStreetCreateBulk returns a new StreetCreateBulk initialized with the given config and builders.
func NewStreetCreateBulk(c Config, builders []*StreetCreate) *StreetCreateBulk {
	return &StreetCreateBulk{Config: c, builders: builders}
}

// NewStreetCreateBulkErr returns a new StreetCreateBulk that returns the given error on Save.
func NewStreetCreateBulkErr(err error) *StreetCreateBulk {
	return &StreetCreateBulk{err: err}
}

// Save creates the Street entities in the database.
func (_c *StreetCreateBulk) Save(ctx context.Context) ([]*Street, error) {
	if _c.err != nil {
		return nil, _c.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(_c.builders))
	nodes := make([]*Street, len(_c.builders))
	mutators := make([]Mutator, len(_c.builders))
	for i := range _c.builders {
		func(i int, root context.Context) {
			builder := _c.builders[i]
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				mutation, ok := m.(*StreetMutation)
				if !ok {
					return nil, fmt.Errorf("unexpected mutation type %T", m)
				}
				if err := builder.check(); err != nil {
					return nil, err
				}
				builder.mutation = mutation
				var err error
				nodes[i], specs[i] = builder.createSpec()
				if i < len(mutators)-1 {
					_, err = mutators[i+1].Mutate(root, _c.builders[i+1].mutation)
				} else {
					spec := &sqlgraph.BatchCreateSpec{Nodes: specs}
					// Invoke the actual operation on the latest mutation in the chain.
					if err = sqlgraph.BatchCreate(ctx, _c.Drv, spec); err != nil {
						if sqlgraph.IsConstraintError(err) {
							err = &ConstraintError{Msg: err.Error(), Wrap: err}
						}
					}
				}
				if err != nil {
					return nil, err
				}
				if specs[i].ID.Value != nil {
					id := specs[i].ID.Value.(int64)
					nodes[i].ID = int(id)
				}
				mutation.SetID(nodes[i].ID)
				mutation.SetDone()
				return nodes[i], nil
			})
			for i := len(builder.hooks) - 1; i >= 0; i-- {
				mut = builder.hooks[i](mut)
			}
			mutators[i] = mut
		}(i, ctx)
	}
	if len(mutators) > 0 {
		if _, err := mutators[0].Mutate(ctx, _c.builders[0].mutation); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

// SaveX is like Save, but panics if an error occurs.
func (_c *StreetCreateBulk) SaveX(ctx context.Context) []*Street {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *StreetCreateBulk) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *StreetCreateBulk) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}
