...
type StreetQuery struct {
	Config
	entbuilder.QueryState[predicate.Street]
	order        []OrderOption
	eagerLoaders map[string]func(context.Context, []*Street) error
	withFKs      bool
}
...
func (_q *StreetQuery) StoreEager(name string, loader func(context.Context, []*Street) error) *StreetQuery {
	if _q.eagerLoaders == nil {
		_q.eagerLoaders = make(map[string]func(context.Context, []*Street) error)
	}
...
func (_q *StreetQuery) EagerLoaders() map[string]func(context.Context, []*Street) error {
...
func (_q *StreetQuery) Order(o ...OrderOption) *StreetQuery {
...
func (_q *StreetQuery) First(ctx context.Context) (*Street, error) {
	_q.Limit(1)
	return entbuilder.RunFirst[*Street, []*Street](ctx, _q, _q.Ctx, ent.OpQueryFirst, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Street, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} })
}
...
func (_q *StreetQuery) Only(ctx context.Context) (*Street, error) {
	_q.Limit(2)
	return entbuilder.RunOnly[*Street, []*Street](ctx, _q, _q.Ctx, ent.OpQueryOnly, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Street, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} }, func(label string) error { return &NotSingularError{Label: label} })
}
...
func (_q *StreetQuery) All(ctx context.Context) ([]*Street, error) {
	return entbuilder.RunAll[[]*Street](ctx, _q, _q.Ctx, ent.OpQueryAll, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Street, error) { return _q.sqlAll(ctx) })
}
...
func (_q *StreetQuery) Clone() *StreetQuery {
	...
	return &StreetQuery{
		...
		eagerLoaders: cloneEagerLoadersStreet(_q.eagerLoaders),
	}
}
...
// dispatch loop.
func (_q *StreetQuery) Fetch(ctx context.Context, hooks ...queryHook) ([]*Street, error) {
	var (
		nodes       = []*Street{}
		withFKs     = _q.withFKs
		_spec       = _q.querySpec()
		loadedTypes = [1]bool{
			_q.eagerLoaders["city"] != nil,
		}
	)
	if _q.eagerLoaders["city"] != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *StreetQuery) IncludeForeignKeys(b bool) *StreetQuery {
	_q.withFKs = b
	return _q
}
...
func (_g *StreetGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*StreetQuery, *StreetGroupBy](ctx, _g.build, _g, _g.build.QueryState.Inters, v)
}
...
func (_s *StreetSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*StreetQuery, *StreetSelect](ctx, _s.StreetQuery, _s, _s.QueryState.Inters, v)
}
...
// streetCreateDescriptor holds the metadata and callbacks for constructing a Street entity.
var streetCreateDescriptor = &entbuilder.CreateDescriptor[Config, Street, *StreetMutation]{
	Table:   Table,
	NewNode: func(c Config) *Street { return &Street{Config: c} },
	ID: &entbuilder.IDDescriptor[Config, Street, *StreetMutation]{
		Column:      FieldID,
		Type:        field.TypeInt,
		UserDefined: false,
		AssignGenerated: func(n *Street, v driver.Value) error {
			id, ok := v.(int64)
			if !ok {
				return fmt.Errorf("unexpected Street.ID type: %T", v)
			}
			n.ID = int(id)
			return nil
		},
...
// cloneEagerLoadersStreet returns a shallow copy of the eager-loaders map.
// The closures themselves are shared between original and clone.
func cloneEagerLoadersStreet(in map[string]func(context.Context, []*Street) error) map[string]func(context.Context, []*Street) error {
	if in == nil {
		return nil
	}
	out := make(map[string]func(context.Context, []*Street) error, len(in))
	for k, v := range in {
		out[k] = v
	}
	return out
