...
var streetUpdateDescriptor = entbuilder.UpdateDescriptor[config, *StreetMutation]{
	Fields: []entbuilder.UpdateFieldDescriptor[*StreetMutation]{
		{
			Column: street.FieldName,
			Type:   field.TypeString,
			Set: func(m *StreetMutation) (driver.Value, bool, error) {
	...
	Edges: []entbuilder.UpdateEdgeDescriptor[config, *StreetMutation]{
		{
			Clear: func(cfg config, m *StreetMutation) (*sqlgraph.EdgeSpec, bool, error) {
				if m.CityCleared() {
			...
			Add: func(cfg config, m *StreetMutation) ([]*sqlgraph.EdgeSpec, error) {
				nodes := m.CityIDs()
				if len(nodes) == 0 {
					return nil, nil
				}
...
func (_u *StreetUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if err := entbuilder.ApplyUpdate(_u.config, _u.mutation, &streetUpdateDescriptor, _spec); err != nil {
		return 0, err
	}
