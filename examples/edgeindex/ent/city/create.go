// Copyright 2019-present Facebook Inc. All rights reserved.
// This source code is licensed under the Apache 2.0 license found
// in the LICENSE file in the root directory of this source tree.

// Code generated by ent, DO NOT EDIT.

package city

import (
	"context"
	"errors"
	"fmt"

	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/runtime/entbuilder"
	"entgo.io/ent/schema/field"
)

// CityCreate is the builder for creating a City entity.
type CityCreate struct {
	Config
	mutation *CityMutation
	hooks    []Hook
}

// NewCityCreate returns a new CityCreate initialized with the given config, hooks, and mutation.
func NewCityCreate(c Config, hooks []Hook, mutation *CityMutation) *CityCreate {
	return &CityCreate{Config: c, hooks: hooks, mutation: mutation}
}

// SetName sets the "name" field.
func (_c *CityCreate) SetName(v string) *CityCreate {
	_ = _c.mutation.SetField("name", v)
	return _c
}

// AddStreetIDs adds the "streets" edge to the Street entity by IDs.
func (_c *CityCreate) AddStreetIDs(ids ...int) *CityCreate {
	_ = _c.mutation.AddEdgeIDs("streets", entbuilder.ToAny(ids)...)
	return _c
}

// Mutation returns the CityMutation object of the builder.
func (_c *CityCreate) Mutation() *CityMutation {
	return _c.mutation
}

// Save creates the City in the database.
func (_c *CityCreate) Save(ctx context.Context) (*City, error) {
	return WithHooks(ctx, _c.sqlSave, _c.mutation, _c.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (_c *CityCreate) SaveX(ctx context.Context) *City {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *CityCreate) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *CityCreate) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}

// check runs all checks and user-defined validators on the builder.
func (_c *CityCreate) check() error {
	if _, ok := entbuilder.GetField[string](_c.mutation, "name"); !ok {
		return &ValidationError{Name: "name", Err: errors.New(`ent: missing required field "City.name"`)}
	}
	return nil
}

func (_c *CityCreate) sqlSave(ctx context.Context) (*City, error) {
	if err := _c.check(); err != nil {
		return nil, err
	}
	_node, _spec := _c.createSpec()
	if err := sqlgraph.CreateNode(ctx, _c.Drv, _spec); err != nil {
		if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{Msg: err.Error(), Wrap: err}
		}
		return nil, err
	}
	id := _spec.ID.Value.(int64)
	_node.ID = int(id)
	_c.mutation.SetID(_node.ID)
	_c.mutation.SetDone()
	return _node, nil
}

func (_c *CityCreate) createSpec() (*City, *sqlgraph.CreateSpec) {
	var (
		_node = &City{Config: _c.Config}
		_spec = sqlgraph.NewCreateSpec(Table, sqlgraph.NewFieldSpec(FieldID, field.TypeInt))
	)
	if value, ok := entbuilder.GetField[string](_c.mutation, "name"); ok {
		_spec.SetField(FieldName, field.TypeString, value)
		_node.Name = value
	}
	if nodes := entbuilder.EdgeIDsAs[int](_c.mutation, "streets"); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   StreetsTable,
			Columns: []string{StreetsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec("id", field.TypeInt),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges = append(_spec.Edges, edge)
	}
	return _node, _spec
}

// CityCreateBulk is the builder for creating many City entities in bulk.
type CityCreateBulk struct {
	Config
	err      error
	builders []*CityCreate
}

// NewCityCreateBulk returns a new CityCreateBulk initialized with the given config and builders.
func NewCityCreateBulk(c Config, builders []*CityCreate) *CityCreateBulk {
	return &CityCreateBulk{Config: c, builders: builders}
}

// NewCityCreateBulkErr returns a new CityCreateBulk that returns the given error on Save.
func NewCityCreateBulkErr(err error) *CityCreateBulk {
	return &CityCreateBulk{err: err}
}

// Save creates the City entities in the database.
func (_c *CityCreateBulk) Save(ctx context.Context) ([]*City, error) {
	if _c.err != nil {
		return nil, _c.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(_c.builders))
	nodes := make([]*City, len(_c.builders))
	mutators := make([]Mutator, len(_c.builders))
	for i := range _c.builders {
		func(i int, root context.Context) {
			builder := _c.builders[i]
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				mutation, ok := m.(*CityMutation)
				if !ok {
					return nil, fmt.Errorf("unexpected mutation type %T", m)
				}
				if err := builder.check(); err != nil {
					return nil, err
				}
				builder.mutation = mutation
				var err error
				nodes[i], specs[i] = builder.createSpec()
				if i < len(mutators)-1 {
					_, err = mutators[i+1].Mutate(root, _c.builders[i+1].mutation)
				} else {
					spec := &sqlgraph.BatchCreateSpec{Nodes: specs}
					// Invoke the actual operation on the latest mutation in the chain.
					if err = sqlgraph.BatchCreate(ctx, _c.Drv, spec); err != nil {
						if sqlgraph.IsConstraintError(err) {
							err = &ConstraintError{Msg: err.Error(), Wrap: err}
						}
					}
				}
				if err != nil {
					return nil, err
				}
				if specs[i].ID.Value != nil {
					id := specs[i].ID.Value.(int64)
					nodes[i].ID = int(id)
				}
				mutation.SetID(nodes[i].ID)
				mutation.SetDone()
				return nodes[i], nil
			})
			for i := len(builder.hooks) - 1; i >= 0; i-- {
				mut = builder.hooks[i](mut)
			}
			mutators[i] = mut
		}(i, ctx)
	}
	if len(mutators) > 0 {
		if _, err := mutators[0].Mutate(ctx, _c.builders[0].mutation); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

// SaveX is like Save, but panics if an error occurs.
func (_c *CityCreateBulk) SaveX(ctx context.Context) []*City {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *CityCreateBulk) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *CityCreateBulk) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}
