...
// LoadUserCars performs the eager-load for the "cars" edge. Body mirrors
// the pre-PR6 *UserQuery.loadCars method, hoisted to root
// so it can reference cross-package types directly.
func LoadUserCars(ctx context.Context, query *car.CarQuery, nodes []*user.User) error {
...
// WithUserCars eager-loads the "cars" edge on a user.UserQuery. The
// optional arguments configure the sibling sub-query before storage.
func WithUserCars(q *user.UserQuery, opts ...func(*car.CarQuery)) *user.UserQuery {
	sub := car.NewCarClient(q.Config).Query()
	for _, opt := range opts {
		opt(sub)
	}
	return q.StoreEager("cars", func(ctx context.Context, parents []*user.User) error {
		return LoadUserCars(ctx, sub, parents)
	})
}

// QueryUserCars returns a car.CarQuery for the "cars" edge of a given user.User.
func QueryUserCars(c *user.UserClient, _m *user.User) *car.CarQuery {
...
// QueryUserCarsFromQuery returns a car.CarQuery that traverses the "cars" edge
// of every user.User matched by q (chained-query form). Mirrors the pre-PR6
// (*user.UserQuery).QueryCars method, hoisted to root so it
// can reference the cross-package car.CarQuery type.
func QueryUserCarsFromQuery(q *user.UserQuery) *car.CarQuery {
