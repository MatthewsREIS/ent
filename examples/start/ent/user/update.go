...
func (_u *UserUpdate) check() error {
	if v, ok := entbuilder.GetField[int](_u.mutation, "age"); ok {
		if err := AgeValidator(v); err != nil {
...
func (_u *UserUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.EdgeCleared("cars") {
	...
	if nodes := _u.mutation.RemovedEdgeIDs("cars"); len(nodes) > 0 && !_u.mutation.EdgeCleared("cars") {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   CarsTable,
	...
	if nodes := _u.mutation.EdgeIDs("cars"); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   CarsTable,
