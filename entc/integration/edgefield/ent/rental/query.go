...
type RentalQuery struct {
	...
	order        []OrderOption
	eagerLoaders map[string]func(context.Context, []*Rental) error
}
...
func (_q *RentalQuery) StoreEager(name string, loader func(context.Context, []*Rental) error) *RentalQuery {
	if _q.eagerLoaders == nil {
		_q.eagerLoaders = make(map[string]func(context.Context, []*Rental) error)
	}
...
func (_q *RentalQuery) EagerLoaders() map[string]func(context.Context, []*Rental) error {
...
func (_q *RentalQuery) Order(o ...OrderOption) *RentalQuery {
...
func (_q *RentalQuery) First(ctx context.Context) (*Rental, error) {
	_q.Limit(1)
	return entbuilder.RunFirst[*Rental, []*Rental](ctx, _q, _q.Ctx, ent.OpQueryFirst, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Rental, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} })
}
...
func (_q *RentalQuery) Only(ctx context.Context) (*Rental, error) {
	_q.Limit(2)
	return entbuilder.RunOnly[*Rental, []*Rental](ctx, _q, _q.Ctx, ent.OpQueryOnly, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Rental, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} }, func(label string) error { return &NotSingularError{Label: label} })
}
...
func (_q *RentalQuery) Clone() *RentalQuery {
	...
	return &RentalQuery{
		...
		eagerLoaders: cloneEagerLoadersRental(_q.eagerLoaders),
	}
}
...
// dispatch loop.
func (_q *RentalQuery) Fetch(ctx context.Context, hooks ...queryHook) ([]*Rental, error) {
	var (
		nodes       = []*Rental{}
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.eagerLoaders["user"] != nil,
			_q.eagerLoaders["car"] != nil,
		}
	)
...
func (_q *RentalQuery) IncludeForeignKeys(bool) *RentalQuery {
	return _q
}
...
func (_q *RentalQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.Ctx.Fields; len(fields) > 0 {
		...
		if _, ok := _q.eagerLoaders["car"]; ok {
			_spec.Node.AddColumnOnce(FieldCarID)
		}
	}
...
// cloneEagerLoadersRental returns a shallow copy of the eager-loaders map.
// The closures themselves are shared between original and clone.
func cloneEagerLoadersRental(in map[string]func(context.Context, []*Rental) error) map[string]func(context.Context, []*Rental) error {
	if in == nil {
		return nil
	}
	out := make(map[string]func(context.Context, []*Rental) error, len(in))
	for k, v := range in {
		out[k] = v
	}
	return out
