...
var postDeleteDescriptor = entbuilder.DeleteDescriptor[config, *PostMutation]{
	Table: post.Table,
	ID: &entbuilder.DeleteIDDescriptor[*PostMutation]{
		Column: post.FieldID,
		Type:   field.TypeInt,
		Value: func(m *PostMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *PostMutation) []func(*sql.Selector) {
...
func (_d *PostDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &postDeleteDescriptor)
	if err != nil {
		return 0, err
	}
