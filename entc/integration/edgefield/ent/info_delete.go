...
var infoDeleteDescriptor = entbuilder.DeleteDescriptor[config, *InfoMutation]{
	Table: info.Table,
	ID: &entbuilder.DeleteIDDescriptor[*InfoMutation]{
		Column: info.FieldID,
		Type:   field.TypeInt,
		Value: func(m *InfoMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *InfoMutation) []func(*sql.Selector) {
...
func (_d *InfoDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &infoDeleteDescriptor)
	if err != nil {
		return 0, err
	}
