...
var rentalDeleteDescriptor = entbuilder.DeleteDescriptor[config, *RentalMutation]{
	Table: rental.Table,
	ID: &entbuilder.DeleteIDDescriptor[*RentalMutation]{
		Column: rental.FieldID,
		Type:   field.TypeInt,
		Value: func(m *RentalMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *RentalMutation) []func(*sql.Selector) {
...
func (_d *RentalDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &rentalDeleteDescriptor)
	if err != nil {
		return 0, err
	}
