...
func (_u *GroupUpdate) gremlin() *dsl.Traversal {
	...
	if value, ok := _u.mutation.Expire(); ok {
		v.Property(dsl.Single, FieldExpire, value)
	}
	...
	if value, ok := _u.mutation.AddedMaxUsers(); ok {
		v.Property(dsl.Single, FieldMaxUsers, __.Union(__.Values(FieldMaxUsers), __.Constant(value)).Sum())
	}
	...
	if _u.mutation.TypeCleared() {
		properties = append(properties, FieldType)
	}
	if _u.mutation.MaxUsersCleared() {
		properties = append(properties, FieldMaxUsers)
	}
	if len(properties) > 0 {
	...
	for _, id := range _u.mutation.RemovedBlockedIDs() {
		tr := rv.Clone().OutE(BlockedLabel).Where(__.OtherV().HasID(id)).Drop().Iterate()
		trs = append(trs, tr)
	}
	for _, id := range _u.mutation.BlockedIDs() {
		v.AddE(BlockedLabel).To(g.V(id)).OutV()
		constraints = append(constraints, &constraint{
			pred: g.E().HasLabel(BlockedLabel).InV().HasID(id).Count(),
			test: __.Is(p.NEQ(0)).Constant(NewErrUniqueEdge(Label, BlockedLabel, id)),
		})
	}
	...
	if _u.mutation.InfoCleared() {
		tr := rv.Clone().OutE(InfoLabel).Drop().Iterate()
		trs = append(trs, tr)
	}
	for _, id := range _u.mutation.InfoIDs() {
		v.AddE(InfoLabel).To(g.V(id)).OutV()
	}
	v.Count()
