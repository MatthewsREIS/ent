...
var petCreateDescriptor = entbuilder.CreateDescriptor[config, Pet, *PetMutation]{
	...
	ID: &entbuilder.IDDescriptor[config, Pet, *PetMutation]{
		...
		AssignGenerated: func(node *Pet, value driver.Value) error {
			...
			default:
				...
				return fmt.Errorf("unexpected Pet.ID type: %T", value)
			}
			return nil
		},
	},
	...
	Fields: []entbuilder.FieldDescriptor[config, Pet, *PetMutation]{
		...
		entbuilder.SimpleField[config, Pet, *PetMutation, uuid.UUID](
			pet.FieldUUID,
			field.TypeUUID,
			(*PetMutation).UUID,
			func(n *Pet, v uuid.UUID) { n.UUID = v },
		),

		entbuilder.SimpleField[config, Pet, *PetMutation, string](
			pet.FieldNickname,
			field.TypeString,
			(*PetMutation).Nickname,
			func(n *Pet, v string) { n.Nickname = v },
		),

		entbuilder.SimpleField[config, Pet, *PetMutation, bool](
			pet.FieldTrained,
			field.TypeBool,
			(*PetMutation).Trained,
			func(n *Pet, v bool) { n.Trained = v },
		),

		entbuilder.SimpleField[config, Pet, *PetMutation, time.Time](
			pet.FieldOptionalTime,
			field.TypeTime,
			(*PetMutation).OptionalTime,
			func(n *Pet, v time.Time) { n.OptionalTime = v },
		),
	},
