...
var cardCreateDescriptor = entbuilder.CreateDescriptor[config, Card, *CardMutation]{
	...
	Fields: []entbuilder.FieldDescriptor[config, Card, *CardMutation]{
		...
		entbuilder.SimpleField[config, Card, *CardMutation, time.Time](
			card.FieldCreateTime,
			field.TypeTime,
			(*CardMutation).CreateTime,
			func(n *Card, v time.Time) { n.CreateTime = v },
		),

		entbuilder.SimpleField[config, Card, *CardMutation, time.Time](
			card.FieldUpdateTime,
			field.TypeTime,
			(*CardMutation).UpdateTime,
			func(n *Card, v time.Time) { n.UpdateTime = v },
		),

		entbuilder.SimpleField[config, Card, *CardMutation, float64](
			card.FieldBalance,
			field.TypeFloat64,
			(*CardMutation).Balance,
			func(n *Card, v float64) { n.Balance = v },
		),
