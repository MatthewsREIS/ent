...
var licenseCreateDescriptor = entbuilder.CreateDescriptor[config, License, *LicenseMutation]{
	...
	ID: &entbuilder.IDDescriptor[config, License, *LicenseMutation]{
		...
		AssignGenerated: func(node *License, value driver.Value) error {
			...
			default:
				...
				return fmt.Errorf("unexpected License.ID type: %T", value)
			}
			return nil
		},
	},
	...
	Fields: []entbuilder.FieldDescriptor[config, License, *LicenseMutation]{
		...
		entbuilder.SimpleField[config, License, *LicenseMutation, time.Time](
			license.FieldCreateTime,
			field.TypeTime,
			(*LicenseMutation).CreateTime,
			func(n *License, v time.Time) { n.CreateTime = v },
		),

		entbuilder.SimpleField[config, License, *LicenseMutation, time.Time](
			license.FieldUpdateTime,
			field.TypeTime,
			(*LicenseMutation).UpdateTime,
			func(n *License, v time.Time) { n.UpdateTime = v },
		),
	},
}
...
func (_c *LicenseCreateBulk) Save(ctx context.Context) ([]*License, error) {
	...
	for i := range _c.builders {
		if err := entgen.ApplyDefaults(_c.builders[i].mutation, licenseCreateSpec.Fields); err != nil {
			return nil, err
		}
	}
	for i := range _c.builders {
