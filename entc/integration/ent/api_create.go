...
var apiCreateDescriptor = entbuilder.CreateDescriptor[config, Api, *APIMutation]{
	...
	ID: &entbuilder.IDDescriptor[config, Api, *APIMutation]{
		...
		Type:        field.TypeInt,
		UserDefined: false,
		AssignGenerated: func(node *Api, value driver.Value) error {
			switch v := value.(type) {
			case int:
				node.ID = int(v)
			case int8:
			...
			default:
				...
				return fmt.Errorf("unexpected Api.ID type: %T", value)
			}
			return nil
		},
	},
...
func (_c *APICreate) sqlSave(ctx context.Context) (*Api, error) {
	...
	_node, _spec, err := entbuilder.BuildCreateSpec(_c.config, _c.mutation, &apiCreateDescriptor)
	if err != nil {
		return nil, err
	}
	...
	if err := entbuilder.ApplyGeneratedID(_c.mutation, _spec, _node, &apiCreateDescriptor); err != nil {
		return nil, err
	}
...
func (_c *APICreateBulk) Save(ctx context.Context) ([]*Api, error) {
	...
	for i := range _c.builders {
		if err := entgen.ApplyDefaults(_c.builders[i].mutation, apiCreateSpec.Fields); err != nil {
			return nil, err
		}
	}
	for i := range _c.builders {
		func(i int, root context.Context) {
			...
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				...
				if err := entgen.CheckCreate(curr.driver.Dialect(), mutation, apiCreateSpec); err != nil {
					return nil, err
				}
				curr.mutation = mutation
				var err error
				nodes[i], specs[i], err = entbuilder.BuildCreateSpec(curr.config, mutation, &apiCreateDescriptor)
				if err != nil {
					return nil, err
				}
				if i < len(mutators)-1 {
					...
					if err == nil {
						for j := range specs {
							if err = entbuilder.ApplyGeneratedID(_c.builders[j].mutation, specs[j], nodes[j], &apiCreateDescriptor); err != nil {
								break
							}
