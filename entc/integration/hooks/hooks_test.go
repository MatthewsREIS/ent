...
func TestSchemaHooks(t *testing.T) {
	...
	defer client.Close()
	err := client.Card.Create().With(card.Field.Number.Set("123")).Exec(ctx)
	require.EqualError(t, err, "card number is too short", "error is returned from hook")
	crd := client.Card.Create().With(card.Field.Number.Set("1234")).SaveX(ctx)
	...
	client.Card.Create().With(card.Field.Number.Set("1234")).SaveX(ctx)
	err = client.Card.Update().Exec(ctx)
	require.EqualError(t, err, "OpUpdate operation is not allowed")

	err = client.User.Update().With(user.Field.Password.Set("pass")).Exec(ctx)
	require.EqualError(t, err, "password cannot be edited on update-many")
	err = client.User.Update().With(user.Field.Password.Clear()).Exec(ctx)
	require.EqualError(t, err, "password cannot be edited on update-many")
}
...
func TestRuntimeHooks(t *testing.T) {
	...
	client.Card.Create().With(card.Field.Number.Set("1234")).SaveX(ctx)
	client.User.Create().With(user.Field.Name.Set("a8m")).SaveX(ctx)
...
func TestRuntimeChain(t *testing.T) {
	...
	client.User.Create().With(user.Field.Name.Set("alexsn")).SaveX(ctx)
...
func TestMutationClient(t *testing.T) {
	...
	crd := client.Card.Create().With(card.Field.Number.Set("1234"), card.Edge.Owner.SetID(a8m.ID)).SaveX(ctx)
...
func TestMutatorClient(t *testing.T) {
		...
		hook.On(
			func(next ent.Mutator) ent.Mutator {
				return hook.CardFunc(func(ctx context.Context, m *ent.CardMutation) (ent.Value, error) {
					...
					m.WhereP(card.Field.ExpiredAt.IsNil())
	...
	c1 := client.Card.Create().With(card.Field.Number.Set("1234")).SaveX(ctx)
	client.Card.DeleteOne(c1).ExecX(ctx)
	expired := client.Card.Query().OnlyX(ctx)
	require.False(t, expired.ExpiredAt.IsZero())

	client.Card.Create().With(card.Field.Number.Set("4567")).ExecX(ctx)
	client.Card.Create().With(card.Field.Number.Set("7890")).ExecX(ctx)
	client.Card.Delete().Where(card.Field.Number.EQ("4567")).ExecX(ctx)
	...
	require.False(t, client.Card.Query().Where(card.Field.ExpiredAt.IsNil()).ExistX(ctx))
}
...
func TestMutationTx(t *testing.T) {
	...
	a8m := tx.User.Create().With(user.Field.Name.Set("a8m")).SaveX(txCtx)
	crd, err := tx.Card.Create().With(card.Field.Number.Set("1234"), card.Edge.Owner.SetID(a8m.ID)).Save(txCtx)
...
func TestDeletion(t *testing.T) {
	...
	client.User.Use(func(next ent.Mutator) ent.Mutator {
		return hook.UserFunc(func(ctx context.Context, m *ent.UserMutation) (ent.Value, error) {
			...
			ent.NewCardClient(*m.Config.(*ent.Config)).Delete().Where(card.Edge.Owner.HasWith(user.Field.ID.EQ(id))).ExecX(ctx)
			return next.Mutate(ctx, m)
		})
	})
	a8m := client.User.Create().With(user.Field.Name.Set("a8m")).SaveX(ctx)
	for i := 0; i < 5; i++ {
		client.Card.Create().With(card.Field.Number.Set(fmt.Sprintf("card-%d", i)), card.Edge.Owner.SetID(a8m.ID)).SaveX(ctx)
	}
...
func TestMutationIDs(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		owner := client.User.Create().With(user.Field.Name.Set(fmt.Sprintf("owner-%d", i))).SaveX(ctx)
		client.Card.Create().With(card.Field.Number.Set(fmt.Sprintf("card-%d", i)), card.Edge.Owner.SetID(owner.ID)).ExecX(ctx)
	}
	for i := 0; i < 5; i++ {
		p := user.And(user.Field.Name.EQ(fmt.Sprintf("owner-%d", i)), user.Edge.Cards.HasWith(card.Field.Number.EQ(fmt.Sprintf("card-%d", i))))
		client.User.Update().With(user.Field.Version.Add(1)).Where(p).ExecX(ctx)
...
func TestPostCreation(t *testing.T) {
	...
	client.Card.Create().With(card.Field.Number.Set("12345"), card.Field.Name.Set("a8m")).SaveX(ctx)
	client.Card.CreateBulk(client.Card.Create().With(card.Field.Number.Set("12345"))).SaveX(ctx)
...
func TestUpdateAfterCreation(t *testing.T) {
	...
	client.User.Use(hook.On(func(next ent.Mutator) ent.Mutator {
		return hook.UserFunc(func(ctx context.Context, m *ent.UserMutation) (ent.Value, error) {
			...
			newUser := ent.NewUserClient(*m.Config.(*ent.Config)).UpdateOne(existingUser).With(user.Field.Version.Set(2)).
				SaveX(ctx)
			return newUser, nil
		})
	}, ent.OpCreate))

	u := client.User.Create().With(user.Field.Name.Set("a8m"), user.Field.Version.Set(1)).SaveX(ctx)
...
func TestUpdateAfterUpdateOne(t *testing.T) {
	...
	client.User.Use(hook.On(func(next ent.Mutator) ent.Mutator {
		return hook.UserFunc(func(ctx context.Context, m *ent.UserMutation) (ent.Value, error) {
			...
			ent.NewUserClient(*m.Config.(*ent.Config)).Update().
				Where(user.Field.ID.EQ(u.ID)).With(user.Field.Version.Set(3)).
				SaveX(ctx)
	...
	u := client.User.Create().With(user.Field.Name.Set("a8m"), user.Field.Version.Set(1)).SaveX(ctx)
	u = client.User.UpdateOne(u).With(user.Field.Version.Set(2)).SaveX(ctx)
...
func TestOldValues(t *testing.T) {
	...
	crd := client.Card.Create().With(card.Field.Number.Set("1234"), card.Field.Name.Set("a8m")).SaveX(ctx)
	client.Card.UpdateOneID(crd.ID).With(card.Field.Name.Set("a8m")).SaveX(ctx)
	...
	err := client.User.UpdateOne(a8m).With(user.Field.Name.Set("Ariel"), user.Field.Version.Set(a8m.Version)).Exec(ctx)
	require.EqualError(t, err, "version field must be incremented by 1")
	a8m = client.User.UpdateOne(a8m).With(user.Field.Name.Set("Ariel"), user.Field.Version.Set(a8m.Version+1)).SaveX(ctx)
...
func TestConditions(t *testing.T) {
	...
	ctx := context.Background()
	crd := client.Card.Create().With(card.Field.Number.Set("9876")).SaveX(ctx)
	crd = client.Card.UpdateOne(crd).With(card.Field.Name.Set("alexsn")).SaveX(ctx)
	crd = client.Card.UpdateOne(crd).With(card.Field.Name.Clear()).SaveX(ctx)
	client.Card.DeleteOne(crd).ExecX(ctx)

	alexsn := client.User.Create().With(user.Field.Name.Set("alexsn")).SaveX(ctx)
	client.User.Update().Where(user.Field.ID.EQ(alexsn.ID)).With(user.Field.Worth.Add(100)).SaveX(ctx)
...
func TestRuntimeTx(t *testing.T) {
	...
	tx, err := client.Tx(ctx)
	require.NoError(t, err)
	tx.Card.Create().With(card.Field.Number.Set("9876")).ExecX(ctx)
...
func TestInterceptor_Sanity(t *testing.T) {
	...
	t.Run("IDs", func(t *testing.T) {
		var (
			...
			users  = client.User.CreateBulk(
				client.User.Create().With(user.Field.Name.Set("a8m")),
				client.User.Create().With(user.Field.Name.Set("nati")),
			).SaveX(ctx)
		)
		defer client.Close()
		client.Intercept(
			...
			intercept.TraverseFunc(func(ctx context.Context, query intercept.Query) error {
				calls++
				query.WhereP(user.Field.ID.EQ(users[0].ID))
				return nil
			}),
		)
	...
	t.Run("GroupBy", func(t *testing.T) {
		...
		client.Intercept(
			...
			intercept.TraverseFunc(func(ctx context.Context, query intercept.Query) error {
				calls++
				query.WhereP(card.Field.Name.NEQ("a8m"))
				return nil
			}),
		)
		client.Card.CreateBulk(
			client.Card.Create().With(card.Field.Name.Set("a8m"), card.Field.Number.Set("1234")),
			client.Card.Create().With(card.Field.Name.Set("a8m"), card.Field.Number.Set("5678")),
			client.Card.Create().With(card.Field.Name.Set("nati"), card.Field.Number.Set("9876")),
		).ExecX(ctx)
		var vs []n2c
...
func TestSoftDelete(t *testing.T) {
	...
	pets := client.Pet.CreateBulk(
		client.Pet.Create().With(pet.Field.Name.Set("a"), pet.Edge.Owner.SetID(a8m.ID)),
		client.Pet.Create().With(pet.Field.Name.Set("b"), pet.Edge.Owner.SetID(a8m.ID)),
		// Set delete_time manually.
		client.Pet.Create().With(pet.Field.Name.Set("c"), pet.Edge.Owner.SetID(a8m.ID), pet.Field.DeleteTime.Set(time.Now())),
	).SaveX(ctx)
...
func TestSharedInterceptor(t *testing.T) {
	...
	client := enttest.Open(t, dialect.SQLite, "file:ent?mode=memory&_fk=1")
	defer client.Close()
	client.User.Create().With(user.Field.Name.Set("a8m")).ExecX(ctx)
	client.User.Create().With(user.Field.Name.Set("nati")).ExecX(ctx)
...
func TestTypedTraverser(t *testing.T) {
	...
	client := enttest.Open(t, dialect.SQLite, "file:ent?mode=memory&_fk=1")
	defer client.Close()
	a8m, nat := client.User.Create().With(user.Field.Name.Set("a8m")).SaveX(ctx), client.User.Create().With(user.Field.Name.Set("nati"), user.Field.Active.Set(false)).SaveX(ctx)
	client.Pet.CreateBulk(
		client.Pet.Create().With(pet.Field.Name.Set("a"), pet.Edge.Owner.SetID(a8m.ID)),
		client.Pet.Create().With(pet.Field.Name.Set("b"), pet.Edge.Owner.SetID(a8m.ID)),
		client.Pet.Create().With(pet.Field.Name.Set("c"), pet.Edge.Owner.SetID(nat.ID)),
	).ExecX(ctx)
	...
	client.User.Intercept(
		intercept.TraverseUser(func(ctx context.Context, q *ent.UserQuery) error {
			q.Where(user.Field.Active.EQ(true))
			return nil
		}),
	)
...
func TestLimitInterceptor(t *testing.T) {
	...
	defer client.Close()
	client.User.Create().With(user.Field.Name.Set("a8m")).SaveX(ctx)
	client.User.Create().With(user.Field.Name.Set("nati")).SaveX(ctx)
	require.Len(t, client.User.Query().AllX(ctx), 2)
	client.Intercept(
		intercept.Func(func(ctx context.Context, q intercept.Query) error {
			q.Limit(1)
