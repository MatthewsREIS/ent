...
func TestEdgeSchemaWithID(t *testing.T) {
	...
	// Create one.
	hub, lab := client.Group.Create().With(group.F.Name.Set("GitHub")).SaveX(ctx), client.Group.Create().With(group.F.Name.Set("GitLab")).SaveX(ctx)
	a8m, nat := client.User.Create().With(user.F.Name.Set("a8m"), user.E.Groups.AddIDs(hub.ID, lab.ID, hub.ID)).SaveX(ctx), client.User.Create().With(user.F.Name.Set("nati"), user.E.Groups.AddIDs(hub.ID, hub.ID)).SaveX(ctx)
	...
	foobar := client.User.CreateBulk(
		client.User.Create().With(user.F.Name.Set("foo"), user.E.Groups.AddIDs(hub.ID, lab.ID, hub.ID, lab.ID)),
		client.User.Create().With(user.F.Name.Set("bar"), user.E.Groups.AddIDs(hub.ID, lab.ID, hub.ID, hub.ID)),
	).SaveX(ctx)
	...
	err = client.Group.UpdateOne(hub).With(group.E.Users.AddIDs(nat.ID)).Exec(ctx)
	...
	err = client.User.UpdateOne(nat).With(user.E.Groups.AddIDs(hub.ID)).Exec(ctx)
	...
	client.UserGroup.Create().With(usergroup.F.UserID.Set(a8m.ID), usergroup.F.GroupID.Set(hub.ID)).OnConflict().Ignore().ExecX(ctx)

	t1 := client.Tag.Create().With(tag.F.Value.Set("tag")).SaveX(ctx)
	client.Group.UpdateOne(hub).With(group.E.Tags.AddIDs(t1.ID)).ExecX(ctx)
...
func TestEdgeSchemaCompositeID(t *testing.T) {
	...
	tweets := client.Tweet.CreateBulk(
		client.Tweet.Create().With(tweet.F.Text.Set("foo")),
		client.Tweet.Create().With(tweet.F.Text.Set("bar")),
		client.Tweet.Create().With(tweet.F.Text.Set("baz")),
	).SaveX(ctx)
	a8m := client.User.Create().With(user.F.Name.Set("a8m"), user.E.LikedTweets.AddIDs(tweets[0].ID, tweets[1].ID)).SaveX(ctx)
	nat := client.User.Create().With(user.F.Name.Set("nati"), user.E.LikedTweets.AddIDs(tweets[0].ID)).SaveX(ctx)
	...
	like := client.TweetLike.Create().With(tweetlike.F.UserID.Set(a8m.ID), tweetlike.F.LikedAt.Set(ts), tweetlike.F.TweetID.Set(tweets[2].ID)).SaveX(ctx)
	...
	nat = client.User.UpdateOne(nat).With(user.E.LikedTweets.AddIDs(like.TweetID)).SaveX(ctx)
	...
	// Ignore update as we already have such edge between a8m and hub.
	client.TweetLike.Create().With(tweetlike.F.UserID.Set(like.UserID), tweetlike.F.TweetID.Set(like.TweetID)).OnConflict().Ignore().ExecX(ctx)
	client.TweetLike.Create().With(tweetlike.F.UserID.Set(like.UserID), tweetlike.F.TweetID.Set(like.TweetID)).OnConflict().DoNothing().ExecX(ctx)

	// Clean all tweet likes and create them in batch again.
	client.TweetLike.Delete().ExecX(ctx)
	likes = client.TweetLike.CreateBulk(
		client.TweetLike.Create().With(tweetlike.F.UserID.Set(a8m.ID), tweetlike.F.TweetID.Set(tweets[0].ID)),
		client.TweetLike.Create().With(tweetlike.F.UserID.Set(a8m.ID), tweetlike.F.TweetID.Set(tweets[1].ID)),
		client.TweetLike.Create().With(tweetlike.F.UserID.Set(nat.ID), tweetlike.F.TweetID.Set(tweets[1].ID)),
		client.TweetLike.Create().With(tweetlike.F.UserID.Set(nat.ID), tweetlike.F.TweetID.Set(tweets[2].ID)),
	).SaveX(ctx)
	...
	affected, err := client.TweetLike.Update().With(tweetlike.F.LikedAt.Set(time.Now())).Save(ctx)
	require.NoError(t, err)
...
func TestEdgeSchemaDefaultID(t *testing.T) {
	...
	tweet1 := client.Tweet.Create().With(tweet.F.Text.Set("foo")).SaveX(ctx)
	tag1 := client.Tag.Create().With(tag.F.Value.Set("1")).SaveX(ctx)
	client.Tweet.UpdateOne(tweet1).With(tweet.E.Tags.AddIDs(tag1.ID)).SaveX(ctx)
	...
	tweet2 := client.Tweet.Create().With(tweet.F.Text.Set("bar"), tweet.E.Tags.AddIDs(tag1.ID)).SaveX(ctx)
...
func TestEdgeSchemaBidiWithID(t *testing.T) {
	...
	a8m := client.User.Create().With(user.F.Name.Set("a8m"), user.E.Friends.AddIDs(nat.ID)).SaveX(ctx)
...
func TestEdgeSchemaBidiCompositeID(t *testing.T) {
	...
	u1 := client.User.Create().With(user.F.Name.Set("u1")).SaveX(ctx)
	u2 := client.User.Create().With(user.F.Name.Set("u2"), user.E.Relatives.AddIDs(u1.ID)).SaveX(ctx)
	u3 := client.User.Create().With(user.F.Name.Set("u3"), user.E.Relatives.AddIDs(u2.ID)).SaveX(ctx)

	err = client.User.UpdateOne(u1).With(user.E.Relatives.AddIDs(u2.ID)).Exec(ctx)
	...
	u4u5 := client.User.CreateBulk(
		client.User.Create().With(user.F.Name.Set("u4"), user.E.Relatives.AddIDs(u1.ID, u2.ID, u3.ID)),
		client.User.Create().With(user.F.Name.Set("u5"), user.E.Relatives.AddIDs(u1.ID, u2.ID, u3.ID)),
	).SaveX(ctx)
	for _, u := range u4u5 {
		...
		err := client.User.UpdateOne(u).With(user.E.Relatives.AddIDs(u3.ID)).Exec(ctx)
		require.True(t, ent.IsConstraintError(err), "duplicate edge error, because edge may contain a different 'weight' value in the database")
		require.EqualError(t, errors.Unwrap(err), "add m2m edge for table relationships: UNIQUE constraint failed: relationships.user_id, relationships.relative_id")
		...
		client.User.UpdateOne(u).With(user.E.Relatives.Clear()).ExecX(ctx)
	...
	info := client.RelationshipInfo.Create().With(relationshipinfo.F.Text.Set("u1->u2")).SaveX(ctx)
	r1 := client.User.QueryRelationship(u1).OnlyX(ctx)
	client.Relationship.UpdateOne(r1).With(relationship.F.InfoID.Set(info.ID)).ExecX(ctx)
...
func TestEdgeSchemaForO2M(t *testing.T) {
	...
	t1 := client.Tweet.Create().With(tweet.F.Text.Set("Hello Edge Schema")).SaveX(ctx)
	a8m := client.User.Create().With(user.F.Name.Set("a8m"), user.E.Tweets.AddIDs(t1.ID)).SaveX(ctx)
	require.Equal(t, t1.ID, client.User.QueryTweets(a8m).OnlyIDX(ctx))
	_, err = client.User.Create().With(user.F.Name.Set("nati"), user.E.Tweets.AddIDs(t1.ID)).Save(ctx)
	require.True(t, ent.IsConstraintError(err), "Tweet can have only one author")

	nat := client.User.Create().With(user.F.Name.Set("nati")).SaveX(ctx)
	err = client.User.UpdateOne(nat).With(user.E.Tweets.AddIDs(t1.ID)).Exec(ctx)
	require.True(t, ent.IsConstraintError(err))
	err = client.UserTweet.Create().With(usertweet.F.UserID.Set(nat.ID), usertweet.F.TweetID.Set(t1.ID)).Exec(ctx)
	require.True(t, ent.IsConstraintError(err))

	tweets := client.Tweet.CreateBulk(
		client.Tweet.Create().With(tweet.F.Text.Set("t1")),
		client.Tweet.Create().With(tweet.F.Text.Set("t2")),
	).SaveX(ctx)
	client.User.UpdateOne(nat).With(user.E.Tweets.AddIDs(tweets[0].ID, tweets[1].ID)).ExecX(ctx)
}
...
func TestEdgeSchemaEntQL(t *testing.T) {
	...
	client.User.UpdateOne(nat).With(user.E.LikedTweets.AddIDs(tweets[0].ID)).ExecX(ctx)
	// Using the regular fluent API.
	...
	u1 := client.User.Create().With(user.F.Name.Set("u1")).SaveX(ctx)
	u2 := client.User.Create().With(user.F.Name.Set("u2"), user.E.Relatives.AddIDs(u1.ID)).SaveX(ctx)
	client.User.Create().With(user.F.Name.Set("u3"), user.E.Relatives.AddIDs(u2.ID)).ExecX(ctx)
	...
	ri := client.RelationshipInfo.Create().With(relationshipinfo.F.Text.Set("parent")).SaveX(ctx)
	r := client.Relationship.Query().FirstX(ctx)
	rl1 := client.Relationship.UpdateOne(r).With(relationship.F.InfoID.Set(ri.ID)).SaveX(ctx)
...
func TestEdgeSchemaTypeMatching(t *testing.T) {
	...
	files := client.File.CreateBulk(
		client.File.Create().With(file.F.Name.Set("a")),
		client.File.Create().With(file.F.Name.Set("b")),
		client.File.Create().With(file.F.Name.Set("c")),
	).SaveX(ctx)
	proc := client.Process.Create().With(process.E.Files.AddIDs(files[0].ID, files[1].ID)).SaveX(ctx)
	...
	af := client.AttachedFile.Create().With(attachedfile.E.Fi.SetID(files[2].ID), attachedfile.F.ProcID.Set(proc.ID)).SaveX(ctx)
