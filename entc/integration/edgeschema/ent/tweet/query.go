...
type TweetQuery struct {
	Config
	entbuilder.QueryState[predicate.Tweet]
	order        []OrderOption
	eagerLoaders map[string]func(context.Context, []*Tweet) error
}
...
func (_q *TweetQuery) StoreEager(name string, loader func(context.Context, []*Tweet) error) *TweetQuery {
	if _q.eagerLoaders == nil {
		_q.eagerLoaders = make(map[string]func(context.Context, []*Tweet) error)
	}
...
func (_q *TweetQuery) EagerLoaders() map[string]func(context.Context, []*Tweet) error {
...
func (_q *TweetQuery) Order(o ...OrderOption) *TweetQuery {
...
func (_q *TweetQuery) First(ctx context.Context) (*Tweet, error) {
	_q.Limit(1)
	return entbuilder.RunFirst[*Tweet, []*Tweet](ctx, _q, _q.Ctx, ent.OpQueryFirst, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Tweet, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} })
}
...
func (_q *TweetQuery) Only(ctx context.Context) (*Tweet, error) {
	_q.Limit(2)
	return entbuilder.RunOnly[*Tweet, []*Tweet](ctx, _q, _q.Ctx, ent.OpQueryOnly, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Tweet, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} }, func(label string) error { return &NotSingularError{Label: label} })
}
...
func (_q *TweetQuery) All(ctx context.Context) ([]*Tweet, error) {
	return entbuilder.RunAll[[]*Tweet](ctx, _q, _q.Ctx, ent.OpQueryAll, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Tweet, error) { return _q.sqlAll(ctx) })
}
...
func (_q *TweetQuery) Clone() *TweetQuery {
	...
	return &TweetQuery{
		...
		eagerLoaders: cloneEagerLoadersTweet(_q.eagerLoaders),
	}
}
...
// dispatch loop.
func (_q *TweetQuery) Fetch(ctx context.Context, hooks ...queryHook) ([]*Tweet, error) {
	var (
		nodes       = []*Tweet{}
		_spec       = _q.querySpec()
		loadedTypes = [6]bool{
			_q.eagerLoaders["liked_users"] != nil,
			_q.eagerLoaders["user"] != nil,
			_q.eagerLoaders["tags"] != nil,
			_q.eagerLoaders["likes"] != nil,
			_q.eagerLoaders["tweet_user"] != nil,
			_q.eagerLoaders["tweet_tags"] != nil,
		}
	)
...
func (_q *TweetQuery) IncludeForeignKeys(bool) *TweetQuery {
	return _q
}
...
func (_g *TweetGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*TweetQuery, *TweetGroupBy](ctx, _g.build, _g, _g.build.QueryState.Inters, v)
}
...
func (_s *TweetSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*TweetQuery, *TweetSelect](ctx, _s.TweetQuery, _s, _s.QueryState.Inters, v)
}
...
// tweetCreateDescriptor holds the metadata and callbacks for constructing a Tweet entity.
var tweetCreateDescriptor = &entbuilder.CreateDescriptor[Config, Tweet, *TweetMutation]{
	Table:   Table,
	NewNode: func(c Config) *Tweet { return &Tweet{Config: c} },
	ID: &entbuilder.IDDescriptor[Config, Tweet, *TweetMutation]{
		Column:      FieldID,
		Type:        field.TypeInt,
		UserDefined: false,
		AssignGenerated: func(n *Tweet, v driver.Value) error {
			id, ok := v.(int64)
			if !ok {
				return fmt.Errorf("unexpected Tweet.ID type: %T", v)
			}
			n.ID = int(id)
			return nil
		},
...
// cloneEagerLoadersTweet returns a shallow copy of the eager-loaders map.
// The closures themselves are shared between original and clone.
func cloneEagerLoadersTweet(in map[string]func(context.Context, []*Tweet) error) map[string]func(context.Context, []*Tweet) error {
	if in == nil {
		return nil
	}
	out := make(map[string]func(context.Context, []*Tweet) error, len(in))
	for k, v := range in {
		out[k] = v
	}
	return out
