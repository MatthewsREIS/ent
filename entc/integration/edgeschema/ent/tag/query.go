...
type TagQuery struct {
	Config
	entbuilder.QueryState[predicate.Tag]
	order        []OrderOption
	eagerLoaders map[string]func(context.Context, []*Tag) error
}
...
func (_q *TagQuery) StoreEager(name string, loader func(context.Context, []*Tag) error) *TagQuery {
	if _q.eagerLoaders == nil {
		_q.eagerLoaders = make(map[string]func(context.Context, []*Tag) error)
	}
...
func (_q *TagQuery) EagerLoaders() map[string]func(context.Context, []*Tag) error {
...
func (_q *TagQuery) Order(o ...OrderOption) *TagQuery {
...
func (_q *TagQuery) First(ctx context.Context) (*Tag, error) {
	_q.Limit(1)
	return entbuilder.RunFirst[*Tag, []*Tag](ctx, _q, _q.Ctx, ent.OpQueryFirst, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Tag, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} })
}
...
func (_q *TagQuery) Only(ctx context.Context) (*Tag, error) {
	_q.Limit(2)
	return entbuilder.RunOnly[*Tag, []*Tag](ctx, _q, _q.Ctx, ent.OpQueryOnly, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Tag, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} }, func(label string) error { return &NotSingularError{Label: label} })
}
...
func (_q *TagQuery) All(ctx context.Context) ([]*Tag, error) {
	return entbuilder.RunAll[[]*Tag](ctx, _q, _q.Ctx, ent.OpQueryAll, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Tag, error) { return _q.sqlAll(ctx) })
}
...
func (_q *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		eagerLoaders: cloneEagerLoadersTag(_q.eagerLoaders),
	}
}
...
// dispatch loop.
func (_q *TagQuery) Fetch(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	var (
		nodes       = []*Tag{}
		_spec       = _q.querySpec()
		loadedTypes = [4]bool{
			_q.eagerLoaders["tweets"] != nil,
			_q.eagerLoaders["groups"] != nil,
			_q.eagerLoaders["tweet_tags"] != nil,
			_q.eagerLoaders["group_tags"] != nil,
		}
	)
...
func (_q *TagQuery) IncludeForeignKeys(bool) *TagQuery {
	return _q
}
...
func (_g *TagGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*TagQuery, *TagGroupBy](ctx, _g.build, _g, _g.build.QueryState.Inters, v)
}
...
func (_s *TagSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*TagQuery, *TagSelect](ctx, _s.TagQuery, _s, _s.QueryState.Inters, v)
}
...
// tagCreateDescriptor holds the metadata and callbacks for constructing a Tag entity.
var tagCreateDescriptor = &entbuilder.CreateDescriptor[Config, Tag, *TagMutation]{
	Table:   Table,
	NewNode: func(c Config) *Tag { return &Tag{Config: c} },
	ID: &entbuilder.IDDescriptor[Config, Tag, *TagMutation]{
		Column:      FieldID,
		Type:        field.TypeInt,
		UserDefined: false,
		AssignGenerated: func(n *Tag, v driver.Value) error {
			id, ok := v.(int64)
			if !ok {
				return fmt.Errorf("unexpected Tag.ID type: %T", v)
			}
			n.ID = int(id)
			return nil
		},
...
// cloneEagerLoadersTag returns a shallow copy of the eager-loaders map.
// The closures themselves are shared between original and clone.
func cloneEagerLoadersTag(in map[string]func(context.Context, []*Tag) error) map[string]func(context.Context, []*Tag) error {
	if in == nil {
		return nil
	}
	out := make(map[string]func(context.Context, []*Tag) error, len(in))
	for k, v := range in {
		out[k] = v
	}
	return out
