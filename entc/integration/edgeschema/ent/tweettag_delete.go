...
var tweettagDeleteDescriptor = entbuilder.DeleteDescriptor[config, *TweetTagMutation]{
	Table: tweettag.Table,
	ID: &entbuilder.DeleteIDDescriptor[*TweetTagMutation]{
		Column: tweettag.FieldID,
		Type:   field.TypeUUID,
		Value: func(m *TweetTagMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *TweetTagMutation) []func(*sql.Selector) {
...
func (_d *TweetTagDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &tweettagDeleteDescriptor)
	if err != nil {
		return 0, err
	}
