...
var usergroupDeleteDescriptor = entbuilder.DeleteDescriptor[config, *UserGroupMutation]{
	Table: usergroup.Table,
	ID: &entbuilder.DeleteIDDescriptor[*UserGroupMutation]{
		Column: usergroup.FieldID,
		Type:   field.TypeInt,
		Value: func(m *UserGroupMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *UserGroupMutation) []func(*sql.Selector) {
...
func (_d *UserGroupDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &usergroupDeleteDescriptor)
	if err != nil {
		return 0, err
	}
