...
var usertweetDeleteDescriptor = entbuilder.DeleteDescriptor[config, *UserTweetMutation]{
	Table: usertweet.Table,
	ID: &entbuilder.DeleteIDDescriptor[*UserTweetMutation]{
		Column: usertweet.FieldID,
		Type:   field.TypeInt,
		Value: func(m *UserTweetMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *UserTweetMutation) []func(*sql.Selector) {
...
func (_d *UserTweetDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &usertweetDeleteDescriptor)
	if err != nil {
		return 0, err
	}
