...
var tweetDeleteDescriptor = entbuilder.DeleteDescriptor[config, *TweetMutation]{
	Table: tweet.Table,
	ID: &entbuilder.DeleteIDDescriptor[*TweetMutation]{
		Column: tweet.FieldID,
		Type:   field.TypeInt,
		Value: func(m *TweetMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *TweetMutation) []func(*sql.Selector) {
...
func (_d *TweetDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &tweetDeleteDescriptor)
	if err != nil {
		return 0, err
	}
