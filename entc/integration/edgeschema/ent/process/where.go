// Copyright 2019-present Facebook Inc. All rights reserved.
// This source code is licensed under the Apache 2.0 license found
// in the LICENSE file in the root directory of this source tree.

// Code generated by ent, DO NOT EDIT.

package process

import (
	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/entc/integration/edgeschema/ent/predicate"
	"entgo.io/ent/runtime/entfield"
)

// Field holds typed predicate/order handles for every comparable field of the Process type.
var Field = struct {
	// ID is the handle for the id field.
	ID entfield.Number[int]
}{
	ID: entfield.NewNumber[int](FieldID, "id"),
}

// Edge holds typed edge handles for every edge of the Process type.
var Edge = struct {
	// Files is the handle for the "files" edge.
	Files entfield.Edge[predicate.File, int]
	// AttachedFiles is the handle for the "attached_files" edge.
	AttachedFiles entfield.Edge[predicate.AttachedFile, int]
}{
	Files:         entfield.NewEdge[predicate.File, int]("files", newFilesStep),
	AttachedFiles: entfield.NewEdge[predicate.AttachedFile, int]("attached_files", newAttachedFilesStep),
}

// And groups predicates with the AND operator between them.
func And(predicates ...predicate.Process) predicate.Process {
	return predicate.Process(sql.AndPredicates(predicates...))
}

// Or groups predicates with the OR operator between them.
func Or(predicates ...predicate.Process) predicate.Process {
	return predicate.Process(sql.OrPredicates(predicates...))
}

// Not applies the not operator on the given predicate.
func Not(p predicate.Process) predicate.Process {
	return predicate.Process(sql.NotPredicates(p))
}
