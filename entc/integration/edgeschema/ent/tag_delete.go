...
var tagDeleteDescriptor = entbuilder.DeleteDescriptor[config, *TagMutation]{
	Table: tag.Table,
	ID: &entbuilder.DeleteIDDescriptor[*TagMutation]{
		Column: tag.FieldID,
		Type:   field.TypeInt,
		Value: func(m *TagMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *TagMutation) []func(*sql.Selector) {
...
func (_d *TagDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &tagDeleteDescriptor)
	if err != nil {
		return 0, err
	}
