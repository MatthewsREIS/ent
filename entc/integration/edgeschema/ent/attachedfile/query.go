...
type AttachedFileQuery struct {
	...
	order        []OrderOption
	eagerLoaders map[string]func(context.Context, []*AttachedFile) error
}
...
func (_q *AttachedFileQuery) StoreEager(name string, loader func(context.Context, []*AttachedFile) error) *AttachedFileQuery {
	if _q.eagerLoaders == nil {
		_q.eagerLoaders = make(map[string]func(context.Context, []*AttachedFile) error)
	}
...
func (_q *AttachedFileQuery) EagerLoaders() map[string]func(context.Context, []*AttachedFile) error {
...
func (_q *AttachedFileQuery) Order(o ...OrderOption) *AttachedFileQuery {
...
func (_q *AttachedFileQuery) First(ctx context.Context) (*AttachedFile, error) {
	_q.Limit(1)
	return entbuilder.RunFirst[*AttachedFile, []*AttachedFile](ctx, _q, _q.Ctx, ent.OpQueryFirst, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*AttachedFile, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} })
}
...
func (_q *AttachedFileQuery) All(ctx context.Context) ([]*AttachedFile, error) {
	nodes, err := entbuilder.RunAll[[]*AttachedFile](ctx, _q, _q.Ctx, ent.OpQueryAll, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*AttachedFile, error) { return _q.sqlAll(ctx) })
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
...
func (_q *AttachedFileQuery) Clone() *AttachedFileQuery {
	...
	return &AttachedFileQuery{
		...
		eagerLoaders: cloneEagerLoadersAttachedFile(_q.eagerLoaders),
	}
}
...
func (_q *AttachedFileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AttachedFile, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.eagerLoaders["fi"] != nil,
			_q.eagerLoaders["proc"] != nil,
		}
	)
...
func (_q *AttachedFileQuery) Fetch(ctx context.Context, hooks ...queryHook) ([]*AttachedFile, error) {
...
func (_q *AttachedFileQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.Ctx.Fields; len(fields) > 0 {
		...
		if _, ok := _q.eagerLoaders["fi"]; ok {
			_spec.Node.AddColumnOnce(FieldFID)
		}
		if _, ok := _q.eagerLoaders["proc"]; ok {
			_spec.Node.AddColumnOnce(FieldProcID)
		}
	}
...
// attachedfileCreateDescriptor holds the metadata and callbacks for constructing a AttachedFile entity.
var attachedfileCreateDescriptor = &entbuilder.CreateDescriptor[Config, AttachedFile, *AttachedFileMutation]{
	Table:   Table,
	NewNode: func(c Config) *AttachedFile { return &AttachedFile{Config: c} },
	ID: &entbuilder.IDDescriptor[Config, AttachedFile, *AttachedFileMutation]{
		Column:      FieldID,
		Type:        field.TypeInt,
		UserDefined: false,
		AssignGenerated: func(n *AttachedFile, v driver.Value) error {
			id, ok := v.(int64)
			if !ok {
				return fmt.Errorf("unexpected AttachedFile.ID type: %T", v)
			}
			n.ID = int(id)
			return nil
		},
...
// cloneEagerLoadersAttachedFile returns a shallow copy of the eager-loaders map.
// The closures themselves are shared between original and clone.
func cloneEagerLoadersAttachedFile(in map[string]func(context.Context, []*AttachedFile) error) map[string]func(context.Context, []*AttachedFile) error {
	if in == nil {
		return nil
	}
	out := make(map[string]func(context.Context, []*AttachedFile) error, len(in))
	for k, v := range in {
		out[k] = v
	}
	return out
