...
var accountDeleteDescriptor = entbuilder.DeleteDescriptor[config, *AccountMutation]{
	Table: account.Table,
	ID: &entbuilder.DeleteIDDescriptor[*AccountMutation]{
		Column: account.FieldID,
		Type:   field.TypeOther,
		Value: func(m *AccountMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *AccountMutation) []func(*sql.Selector) {
...
func (_d *AccountDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &accountDeleteDescriptor)
	if err != nil {
		return 0, err
	}
