...
var noteDeleteDescriptor = entbuilder.DeleteDescriptor[config, *NoteMutation]{
	Table: note.Table,
	ID: &entbuilder.DeleteIDDescriptor[*NoteMutation]{
		Column: note.FieldID,
		Type:   field.TypeString,
		Value: func(m *NoteMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *NoteMutation) []func(*sql.Selector) {
...
func (_d *NoteDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &noteDeleteDescriptor)
	if err != nil {
		return 0, err
	}
