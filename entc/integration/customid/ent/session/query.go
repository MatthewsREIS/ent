...
type SessionQuery struct {
	Config
	entbuilder.QueryState[predicate.Session]
	order        []OrderOption
	eagerLoaders map[string]func(context.Context, []*Session) error
	withFKs      bool
}
...
func (_q *SessionQuery) StoreEager(name string, loader func(context.Context, []*Session) error) *SessionQuery {
	if _q.eagerLoaders == nil {
		_q.eagerLoaders = make(map[string]func(context.Context, []*Session) error)
	}
...
func (_q *SessionQuery) EagerLoaders() map[string]func(context.Context, []*Session) error {
...
func (_q *SessionQuery) Order(o ...OrderOption) *SessionQuery {
...
func (_q *SessionQuery) First(ctx context.Context) (*Session, error) {
	_q.Limit(1)
	return entbuilder.RunFirst[*Session, []*Session](ctx, _q, _q.Ctx, ent.OpQueryFirst, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Session, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} })
}
...
func (_q *SessionQuery) Only(ctx context.Context) (*Session, error) {
	_q.Limit(2)
	return entbuilder.RunOnly[*Session, []*Session](ctx, _q, _q.Ctx, ent.OpQueryOnly, Label, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Session, error) { return _q.sqlAll(ctx) }, func(label string) error { return &NotFoundError{Label: label} }, func(label string) error { return &NotSingularError{Label: label} })
}
...
func (_q *SessionQuery) All(ctx context.Context) ([]*Session, error) {
	return entbuilder.RunAll[[]*Session](ctx, _q, _q.Ctx, ent.OpQueryAll, _q.QueryState.Inters, _q.prepareQuery, func(ctx context.Context) ([]*Session, error) { return _q.sqlAll(ctx) })
}
...
func (_q *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		eagerLoaders: cloneEagerLoadersSession(_q.eagerLoaders),
	}
}
...
// dispatch loop.
func (_q *SessionQuery) Fetch(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	var (
		nodes       = []*Session{}
		withFKs     = _q.withFKs
		_spec       = _q.querySpec()
		loadedTypes = [1]bool{
			_q.eagerLoaders["device"] != nil,
		}
	)
	if _q.eagerLoaders["device"] != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *SessionQuery) IncludeForeignKeys(b bool) *SessionQuery {
	_q.withFKs = b
	return _q
}
...
func (_q *SessionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(Table, Columns, sqlgraph.NewFieldSpec(FieldID, field.TypeBytes))
	_spec.From = _q.Sql
...
func (_g *SessionGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*SessionQuery, *SessionGroupBy](ctx, _g.build, _g, _g.build.QueryState.Inters, v)
}
...
func (_s *SessionSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*SessionQuery, *SessionSelect](ctx, _s.SessionQuery, _s, _s.QueryState.Inters, v)
}
...
// sessionCreateDescriptor holds the metadata and callbacks for constructing a Session entity.
var sessionCreateDescriptor = &entbuilder.CreateDescriptor[Config, Session, *SessionMutation]{
	Table:   Table,
	NewNode: func(c Config) *Session { return &Session{Config: c} },
	ID: &entbuilder.IDDescriptor[Config, Session, *SessionMutation]{
		...
		AssignNode: func(n *Session, fv entbuilder.FieldValue) error {
			n.ID = fv.Node.(schema.ID)
			return nil
		},
		AssignGenerated: func(n *Session, v driver.Value) error {
			switch x := v.(type) {
			case schema.ID:
				n.ID = x
			default:
				return fmt.Errorf("unexpected Session.ID type: %T", v)
			}
			return nil
		},
	},
...
// cloneEagerLoadersSession returns a shallow copy of the eager-loaders map.
// The closures themselves are shared between original and clone.
func cloneEagerLoadersSession(in map[string]func(context.Context, []*Session) error) map[string]func(context.Context, []*Session) error {
	if in == nil {
		return nil
	}
	out := make(map[string]func(context.Context, []*Session) error, len(in))
	for k, v := range in {
		out[k] = v
	}
	return out
