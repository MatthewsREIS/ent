...
var conversionDeleteDescriptor = entbuilder.DeleteDescriptor[config, *ConversionMutation]{
	Table: conversion.Table,
	ID: &entbuilder.DeleteIDDescriptor[*ConversionMutation]{
		Column: conversion.FieldID,
		Type:   field.TypeInt,
		Value: func(m *ConversionMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *ConversionMutation) []func(*sql.Selector) {
...
func (_d *ConversionDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &conversionDeleteDescriptor)
	if err != nil {
		return 0, err
	}
