...
func (c *ZooClient) Create() *ZooCreate {
	...
	return NewZooCreate(c.Config, c.Hooks(), mutation)
}
...
func (c *ZooClient) CreateBulk(builders ...*ZooCreate) *ZooCreateBulk {
	return NewZooCreateBulk(c.Config, builders)
}
...
func (c *ZooClient) MapCreateBulk(slice any, setFunc func(*ZooCreate, int)) *ZooCreateBulk {
	...
	if rv.Kind() != reflect.Slice {
		return NewZooCreateBulkErr(fmt.Errorf("calling to ZooClient.MapCreateBulk with wrong type %T, need slice", slice))
	}
...
func (c *ZooClient) Delete() *ZooDelete {
	...
	return NewZooDelete(c.Config, c.Hooks(), mutation)
}
...
func (c *ZooClient) Mutate(ctx context.Context, m *ZooMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return NewZooCreate(c.Config, c.Hooks(), m).Save(ctx)
	case OpUpdate:
		return NewZooUpdate(c.Config, c.Hooks(), m).Save(ctx)
	case OpUpdateOne:
		return NewZooUpdateOne(c.Config, c.Hooks(), m).Save(ctx)
	case OpDelete, OpDeleteOne:
		return NewZooDelete(c.Config, c.Hooks(), m).Exec(ctx)
	default:
