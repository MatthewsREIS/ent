...
var zooDeleteDescriptor = entbuilder.DeleteDescriptor[config, *ZooMutation]{
	Table: zoo.Table,
	ID: &entbuilder.DeleteIDDescriptor[*ZooMutation]{
		Column: zoo.FieldID,
		Type:   field.TypeInt,
		Value: func(m *ZooMutation) (driver.Value, bool, error) {
			if id, ok := m.ID(); ok {
				return id, true, nil
			}
	...
	Predicates: func(m *ZooMutation) []func(*sql.Selector) {
...
func (_d *ZooDelete) sqlExec(ctx context.Context) (int, error) {
	_spec, err := entbuilder.BuildDeleteSpec(_d.config, _d.mutation, &zooDeleteDescriptor)
	if err != nil {
		return 0, err
	}
