...
func importAliases(file *ast.File, manifest Manifest) map[string]string {
...
// rewriteAST walks file, rewriting every pkg.Sel selector (call position or
// bare value position) whose pkg resolves to a manifest entry and whose Sel
// decomposes into a field/edge + op, into the nested pkg.F.Name.Op /
// pkg.E.Name.Op form. It reports whether any rewrite was made.
func rewriteAST(file *ast.File, manifest Manifest) bool {
	aliases := importAliases(file, manifest)
	if len(aliases) == 0 {
		return false
	}
	changed := false
...
// filename is used only for parse error messages / position info. It
// returns the (possibly unchanged) source, whether any rewrite was made,
// and a parse error if src isn't valid Go.
func RewriteSource(filename string, src []byte, manifest Manifest) ([]byte, bool, error) {
...
func ProcessFile(path string, manifest Manifest) (bool, error) {
	src, err := os.ReadFile(path)
	if err != nil {
		return false, err
	}
	out, changed, err := RewriteSource(path, src, manifest)
	if err != nil || !changed {
		return false, err
	}
...
func ProcessDirs(dirs []string, manifest Manifest) ([]string, error) {
	...
	for _, dir := range dirs {
		err := filepath.WalkDir(dir, func(p string, d fs.DirEntry, err error) error {
			...
			changed, err := ProcessFile(p, manifest)
			if err != nil {
				return err
			}
			if changed {
