...
(Note: Task 2's matrix marks `api-graphql/src/ent/schema` with mutation ✓; verification shows those are calls to schema-definition `Fields()` via mixin (e.g., `BaseMixin{}.Fields()`), not generated mutation-state APIs — the zero-coverage conclusion stands.)
...
- **Also add to:** `entc/integration/` — extend the root-package test suite, either by adding a new sub-test in `integration_test.go` or by creating a new `entc/integration/mutation_test.go` in package `integration`. Exercise `.Mutation().IDs()`, `.Mutation().Op()`, `FieldCleared(name)`, and `AddedField(name)` across at least two entity types; if covering hook-layer wiring, also exercise the `hook.HasFields(...)` hook combinator.
...
- **Add to:** `entc/integration/edgeschema/edgeschema_test.go` — this scenario already calls entql filter methods on edge entities; add 2–3 additional sub-tests covering entql predicates on non-edge entities (e.g., `WhereName` on `GroupFilter`/`RoleFilter`, `WhereText` on `TweetFilter`, `WhereHasLikesWith` on `TweetFilter`/`UserFilter`) to give at least a second independently-structured entql scenario.
...
Task 1 notes: "Tx/client hooks are thin outside root and hooks. Only `root` and `hooks` call `client.Tx()` directly or register global hooks via `client.Use()`." The `template` scenario exercises an entity-level hook via `client.User.Use()` (not the global `client.Use()`), so template does not reach the top-level client-hook surface. The consumer matrix shows 5 directories exercising client surfaces, but `txutil` only tests the `gen.Tx` interface (not entity-level transactions), and `entcontext_test` only tests context helpers. Direct `client.Tx()` usage in a consumer test that runs assertions inside the transaction body is rare.
